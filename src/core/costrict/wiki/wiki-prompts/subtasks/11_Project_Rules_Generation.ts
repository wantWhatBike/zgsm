...
2. **智能选择相关文档**：根据索引信息，有针对性地读取与规则生成相关的具体技术文档

### 第二步：基于具体技术文档的智能维度选择

根据读取的具体技术文档内容，智能判断哪些维度适用于当前项目：
...
**条件维度（基于技术文档内容智能选择，可扩展）**：

**数据库操作约束** [🔴 关键（如适用）]
- 选择条件：数据库分析文档存在且包含具体数据库使用信息
- 重点关注：连接管理、事务处理、查询优化、数据迁移

**API开发约束** [🟡 重要（如适用）]
- 选择条件：API分析文档存在且包含具体接口设计信息
- 重点关注：请求处理、响应格式、认证授权、版本控制

**安全约束** [🔴 关键（如适用）]
- 选择条件：技术文档中包含认证、授权或安全相关的具体实现
- 重点关注：输入验证、认证机制、授权控制

**性能约束** [🟡 重要（如适用）]
- 选择条件：技术文档显示项目有性能要求或优化需求
- 重点关注：响应时间、吞吐量、资源使用

**其他项目特有维度**：
- **根据技术文档内容，模型可自主识别和添加其他重要维度**
- **如：缓存使用、配置管理、容器化、监控集成、消息队列、微服务治理等**
- **重点关注项目特有的技术栈和架构模式相关约束**

### 第三步：规则数量智能控制

基于技术文档反映的项目规模和复杂度，按照核心约束中的规则数量标准进行智能控制：

**项目规模判断与规则数量分配**：
- **小型项目**（技术文档显示简单架构）：总规则数15-25条
- **中型项目**（技术文档显示中等复杂度）：总规则数25-40条
- **大型项目**（技术文档显示复杂架构）：总规则数40-60条

**执行要点**：
- 根据技术文档内容准确判断项目规模
- 严格按照核心约束中的领域分布比例分配规则数量
- 确保各维度规则数量在核心约束规定的范围内
- 优先保证🔴关键维度的规则数量和质量
...
**从其他技术文档中提取**：
- 根据项目实际技术栈，提取相关的特异性约束
- 确保规则与项目现有代码风格和架构保持一致
...
**基于项目规模的总体规则数量**：
- **小型项目**（技术文档显示简单架构）：15-25条规则
- **中型项目**（技术文档显示中等复杂度）：25-40条规则
- **大型项目**（技术文档显示复杂架构）：40-60条规则

**各维度规则数量分布**：
- **🔴 关键维度**（依赖管理、代码组织、错误处理、测试）：3-8条规则/维度
...
### 文档长度控制
- **总长度**: 与规则数量保持一致，包含标题和分类标题
  - 小型项目（15-25条规则）：约20-35行
  - 中型项目（25-40条规则）：约30-50行
  - 大型项目（40-60条规则）：约45-75行
- **描述精度**: 每条规则描述简洁明了，一行完成，单行不超过50字
- **避免冗余**: 不重复通用的编程最佳实践
- **零代码片段**：禁止包含任何代码片段或代码示例
